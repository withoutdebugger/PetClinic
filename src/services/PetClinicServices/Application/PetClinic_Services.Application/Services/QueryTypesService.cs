...
{
    public class QueryTypesService : IQueryTypesService
    {
        #region Atributtes
        ...
        #region Constructor
        public QueryTypesService(petsContext context)
        {
            _context = context;
        }
        #endregion
        ...
        public IEnumerable<SelectView> GetAll()
        {
            try
            {
                return (from c in _context.QueryTypes.ToList()
                                       select new SelectView
                                       {
                                           Value = c.QueryTypes1.ToString(),
                                           Label = c.Description
                                       }).ToList();
            }
            catch (Exception e)
